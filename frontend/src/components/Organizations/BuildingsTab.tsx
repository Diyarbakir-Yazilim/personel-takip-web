...
import { Building, deleteBuilding, getBuildings, createBuilding, updateBuilding } from "@/services/organizations";
...
export function BuildingsTab({ isActive }: { isActive: boolean }) {
  ...
  async function loadData() {
    ...
    try {
      ...
      setError(err.message || "Binalar yüklenemedi");
    } finally {
      setLoading(false);
    }
  }
  ...
  async function handleSave(e: React.FormEvent) {
    ...
    try {
      ...
      setError(err.message || "Bina kaydedilemedi");
    } finally {
  ...
  async function handleDelete(id: string) {
    ...
    try {
      ...
      alert(err.message || "Bina silinemedi");
    }
  }
  ...
  return (
    <div className="space-y-4">
      <div className="flex justify-between items-center">
        <h2 className="text-xl font-semibold">Binalar</h2>
        <Button onClick={openCreate}>
          <Plus className="mr-2 h-4 w-4" /> Bina Ekle
        </Button>
      </div>
      ...
      <Dialog open={isDialogOpen} onOpenChange={setIsDialogOpen}>
        <DialogContent>
          <DialogHeader>
            <DialogTitle>{editingId ? "Bina Düzenle" : "Bina Ekle"}</DialogTitle>
          </DialogHeader>
